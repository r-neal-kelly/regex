...
inline void_t test_allocator_heap_i()
{
    ...
    {
        bool_t worked = ALLOCATOR_HEAP_i.allocate(&ptr, 0xFFF);
        test(worked);
    ...
    {
        ALLOCATOR_HEAP_i.deallocate(&ptr);
        test(!pointer_is_valid(&ptr));
    }
    test_end();
    ...
    test_begin(L"allocator_heap_i: cannot allocate MAX_WORD, and thus should invalidate pointer.");
    {
        ptr.bytes = (byte_t*)1;
        ptr.byte_count = 1;
        bool_t worked = ALLOCATOR_HEAP_i.allocate(&ptr, MAX_WORD);
        test(!worked);
        test(!pointer_is_valid(&ptr));
        test(ptr.bytes == 0);
    ...
    {
        pointer_t ptr;
        ALLOCATOR_HEAP_i.allocate(&ptr, 0xFF);
        test(pointer_is_valid(&ptr));
        test(ptr.bytes);
        test(ptr.byte_count == 0xFF);
        bool_t did_reallocate = ALLOCATOR_HEAP_i.reallocate(&ptr, 0xFFFF);
        test(did_reallocate);
        test(pointer_is_valid(&ptr));
