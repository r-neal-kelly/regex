...
void_t charcoder_utf_16_le_read_forward(const void_t* from, string_subsequence_t* result)
{
    assert(from);
    assert(result);
...
void_t charcoder_utf_16_le_to_subsequence(u32_t point, string_subsequence_t* result)
{
    assert(result);
...
charcoder_utf_16_le_t CHARCODER_UTF_16_LE = DEFINE_CHARCODER_i(charcoder_utf_16_le);
